...
class CATNetTrainDataset(Dataset):
    def __init__(self, args):
        super(CATNetTrainDataset, self).__init__()
        self.args = args
        self.rs_ids = []
        self.hazy_ids = []
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            if de_id == 3:
                ...
                degrad_img = crop_img(
                    np.array(Image.open(os.path.join(self.args.gopro_dir, 'blur/', sample["clean_id"])).convert('RGB')),
                    base=16)
                clean_img = crop_img(np.array(
                    Image.open(os.path.join(self.args.gopro_dir, 'sharp/', sample["clean_id"])).convert('RGB')),
                                     base=16)
                clean_name = self._get_deblur_name(sample["clean_id"])
            elif de_id == 6:
                # Enhancement with LOL training set
                degrad_img = crop_img(np.array(
                    Image.open(os.path.join(self.args.enhance_dir, 'low/', sample["clean_id"])).convert('RGB')),
                                      base=16)
                clean_img = crop_img(
                    np.array(Image.open(os.path.join(self.args.enhance_dir, 'gt/', sample["clean_id"])).convert('RGB')),
                    base=16)
...
class DerainDehazeDataset(Dataset):
    ...
    def _get_gt_path(self, degraded_name):
        if self.task_idx == 0:
            basename = os.path.basename(degraded_name)
            norain_name = 'norain-' + basename.split('rain-')[1]
            gt_name = os.path.join(os.path.dirname(degraded_name).replace("input", "target"), norain_name)
        elif self.task_idx == 1:
